...
for i, name in enumerate(bucketNames):
        bucket_name = ("%s%s" %(args.targetStem, name))

        print("[*] %d/%d: Testing bucket: %s" %(i+1, lineCount, bucket_name))
        r = requests.head("http://%s.s3.amazonaws.com" % (bucket_name))
        if r.status_code != 404:
            ...
            check = subprocess.getoutput("/usr/local/bin/aws s3 ls s3://%s%s" % (args.targetStem, name))
            print(check)
        else:
            sys.stdout.write('')
